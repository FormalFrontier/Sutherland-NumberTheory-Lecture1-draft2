...
7. `A` is a Dedekind domain that is not a field.
...
Condition 7 ("maximal noetherian ring of dimension one" in the PDF) is formalized as
`IsDedekindDomain A ∧ IsLocalRing A ∧ ¬IsField A`. In classical commutative algebra,
"maximal" refers to a maximal order (i.e., integrally closed), and a noetherian integrally
closed domain of dimension one is a Dedekind domain. Locality is needed for DVR equivalence.

Condition 5 ("regular noetherian local ring of dimension one") is formalized using the
property that every nonzero ideal is a power of the maximal ideal, which for local rings
of dimension one is equivalent to regularity.
-/
