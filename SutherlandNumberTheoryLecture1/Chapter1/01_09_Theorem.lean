...
We also give a direct proof for `ℚ` using the fundamental theorem of arithmetic.
-/

/-- **Theorem 1.9** (Product Formula, general). For nonzero `x : K`, the product of `|x|_v`
...
/-- **Theorem 1.9** (Product Formula for ℚ). For any nonzero rational `q`, the product of the
