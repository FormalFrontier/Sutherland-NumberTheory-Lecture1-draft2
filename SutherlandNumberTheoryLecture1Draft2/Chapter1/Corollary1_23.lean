...
theorem sutherland_corollary1_23 (A : Type*) [CommRing A] [IsDomain A]
  ...
  -- UniqueFactorizationMonoid → NormalizedGCDMonoid → Nonempty (GCDMonoid) → IsIntegrallyClosed
  inferInstance
...
theorem sutherland_corollary1_23_pid (A : Type*) [CommRing A] [IsDomain A]
    ...
    IsIntegrallyClosed A :=
  -- IsPrincipalIdealRing → IsBezout → Nonempty (GCDMonoid) → IsIntegrallyClosed
  inferInstance
