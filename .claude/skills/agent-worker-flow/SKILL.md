...
**Verify issue closure after merge:** `coordination create-pr` closes the issue via
"Closes #N" in the PR body — but only one PR can close an issue on merge. If another
PR already closed the issue, or the auto-close silently failed, the issue stays open
and attracts duplicate work. After your PR merges, confirm:
```bash
